...
Starship, Neovim, and Docker. Running without module flags installs the full setup.
...
nodejs.org, npm, PyPI, and the Oh My Zsh/Starship installers.
...
- `--only` and `--skip` accept `git,zsh,starship,nvim,docker`; skip wins if both
...
Debian Starship installation use their own update mechanisms. Update archive
