...
distro_install_docker() {
  ...
  if [ ! -f /etc/apt/keyrings/docker.gpg ]; then
    if [ "${DRY_RUN:-0}" = "1" ]; then
      ...
      make_temp
      key_file="$TEMP_FILE"
      curl -fsSL -o "$armored_key" https://download.docker.com/linux/debian/gpg
  ...
  codename="$(
    . /etc/os-release
    if [ "${DRY_RUN:-0}" = 1 ] && [ "${ID:-}" != debian ]; then
      printf '<debian-codename>'
      exit 0
    fi
    printf "%s" "${VERSION_CODENAME:?Missing VERSION_CODENAME in /etc/os-release}"
  )"
