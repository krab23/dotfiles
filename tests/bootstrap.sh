...
XDG_CONFIG_HOME="$HOME/custom config" install --distro debian --only nvim,starship --config-only
no_commands
...
: > "$MOCK_LOG"
install --distro arch --skip git,zsh,starship,nvim,docker
no_commands
...
printf 'ok: dry-run works with missing zsh/starship/curl binaries\n'
